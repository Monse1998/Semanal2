...
while True:
    ...
    ala_izquierda.rotate(angle=rotacion, axis=vector(0, 1, 0), origin=cuerpo1.pos)
    ala_derecha.rotate(angle=-rotacion, axis=vector(0, 1, 0), origin=cuerpo1.pos)
    ...
    if angulo_circular > math.pi * 0.6 or angulo_circular < -math.pi * 0.6:
        ...
        cabeza.rotate(angle=rotacion, axis=vector(0,1,0), origin=cabeza.pos)
        ojo1.rotate(angle=rotacion, axis=vector(0,1,0), origin=cabeza.pos)
        ojo2.rotate(angle=rotacion, axis=vector(0,1,0), origin=cabeza.pos)
        pico.rotate(angle=rotacion, axis=vector(0,1,0), origin=cabeza.pos)
